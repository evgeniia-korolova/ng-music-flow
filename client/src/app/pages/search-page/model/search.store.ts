      ...
      loadSearchResults: rxMethod<{ query: string; filters: SearchFiltersState; offset: number }>(
        pipe(
          ...
          switchMap(({ query, filters, offset }) => {
            ...
            if (filters.sortBy === 'popularity') apiParams['boost'] = 'listens_total';
            ...
            // if(filters.sortBy === 'title') apiParams['order'] = 'name';
            // if(filters.sortBy === 'artist') apiParams['order'] = 'artist_name';
...
export const SearchStoreDuble = signalStore(
  withState(extendedInitialState),
  withComputed((store) => {
